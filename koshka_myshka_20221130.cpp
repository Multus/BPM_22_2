...
int catch_attempt(
	...
	long current_step = -1;*/
	static int limit = 0;
	std::cout << /*std::endl <<  */"############# Catch started! speed to speed: " << norm(cat_speed) / norm(mouse_speed) << std::endl;
	if(limit > 40){return 13;} //если плохо сходится, обрубаем исполнение
	limit += 1;
	static double dt = 0.001;
	for (; mouse != cat && mouse.y <= epsilon * 2; t += dt){
		mouse = mouse + (mouse_speed + additional_mouse_speed(mode, cat, mouse, dt)) * dt; //мышь пока бежит не к норке, а просто вверх, пофикситбб
...
Rdec2D additional_mouse_speed(std::string mode, const Rdec2D& cat, const Rdec2D& mouse, const double dt){
    if(mode == "classic"){
        ...
        static Rdec2D cumulative_speed = {0,0};
        cumulative_speed = cumulative_speed + 0.5 * ((mouse - cat) * (1 / (norm(mouse - cat) + 1)) * dt);
        /* std::cout << "Speeding up! " << cumulative_speed.x << ", " << cumulative_speed.y << std::endl; */
        return cumulative_speed;
    }
    else{
...
int catch_attempt(Rdec2D cat, Rdec2D mouse, Rdec2D mouse_speed, Rdec2D cat_speed, 
double touch_bound = epsilon * 2, std::string mode = "classic"){
	double t = 0.0;
    /*const double finish_t = norm(mouse)/norm(mouse_speed);
	long current_step = -1;*/
	...
	static double dt = 0.001;
	for (; mouse != cat && mouse.y <= epsilon * 2; t += dt){
		mouse = mouse + (mouse_speed + additional_mouse_speed(mode, cat, mouse, dt)) * dt;
		cat = cat + cat_speed_vector(cat, mouse, norm(cat_speed), cat_speed) * dt;
		if(norm(mouse - cat) < norm((cat_speed - mouse_speed) * dt)){
		    cat = mouse;
		    dt /= 2;
...
//не, я понимаю, что некорректно выходит… но технические шоколадки
/* #include <cmath>
#include <numbers>
#include <limits>
#include <iostream>
